...
my_list = []
while True:
    my_word = input("Введите число чтобы продолжить или слово stop для отмены: ")
    if my_word == "stop":
        break
    else:
        try:
            if my_word.isdigit() is False:
                raise NumberOrNot("Вы ввели текст!")
            else:
                my_list.append(int(my_word))
        except NumberOrNot as err:
            print(err)
